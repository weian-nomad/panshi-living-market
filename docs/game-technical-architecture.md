...
本文件是《盤勢・市中人》的工程契約。產品規則以[產品企劃書](./game-product-proposal.md)為準；旅程與跨職能討論放在 [FigJam](https://www.figma.com/board/OZG06ChjZGMaatLc2DDzQA)。若三者衝突，先修產品企劃，再改本文件與 FigJam，不能讓實作自行發明規則。

## 1. 首版定義

這裡的 MVP 是 **Minimum Final Product**。首個 200 人公開封測版就要具備最終產品的資料形狀、身份系統、事件模型、核心循環、雙端契約、隱私操作、視覺語言與營運能力。封測不啟用付款、廣告與尚未通過法規審查的當期個股功能；這些能力由 feature flag 關閉，不用臨時資料表或假流程代替。

首發後允許的工作只有三類：

1. 增加世界內容、角色、歷史節點與敘事細節。
...
| 一個事件、五個角色就算完成 | 首發含 20 個封存歷史節點、完整角色引擎與五人每週循環 |
| 只做手機直式觀測室 | 同時交付 mobile、desktop、reduced-motion 與既有 SwiftUI 必要畫面 |
...
**《盤勢・市中人》使用獨立公開 repository，以 pnpm workspace＋Turborepo 組成 modular monolith；它只接收盤勢研究產品輸出的版本化 sealed fact contract，部署為可獨立擴容的 Web、simulation worker、world scheduler 與 Remotion renderer。**
...
| Feature flags | PostgreSQL `feature_flags`＋`feature_flag_overrides` | 預設 fail closed；付款、廣告、live market 全部關閉 |
...
POST /api/v1/simulation-runs
  → transaction: consume weekly slot
                 create run
...
| `weekly_slots` | consume-once | unique(user_id, season_id, week, slot_no)；reservation idempotent |
| `entitlements` | 事件＋projection | source adapter、effective interval、capabilities；beta 也是正式 entitlement |
| `commons_entitlements` | append＋projection | relinquish、public、recall 狀態機與生效時間 |
...
export const AllowedAction = z.object({
  kind: z.enum([
    ...
    "update_memory_interpretation", "defer",
  ]),
...
`model_registry` 保存 capability alias、輸入／輸出 schema hash、context limit、locale 能力、啟停狀態與 fallback chain。角色出生時選的是公開 alias；不顯示或保存非公開 capacity 來源。預設思考核心為企劃指定的 `gemma-4-26b-a4b-it` alias，實際 route 只存在私密營運設定。
...
失去權益的狀態機完整可測：grace → guidance_lost → commons_eligible → public_after_consent → recalled。私人筆記、未公開人生聖經與帳號資料永遠不隨角色進公地。
...
| SEO | self canonical、sitemap、robots、OG；帳號、觀測室、分享草稿 noindex |
...
| 觀測室 route incremental JS gzip | ≤ 250 KB；角色圖片不計 |
...
首發必要畫面：登入與 link exchange、今日／20 個歷史節點入口、角色名冊與出生、五席安排、觀測室、三層內心戲、證據因果、關係網、人物週卷、私人筆記、公地、模型選擇、權益模擬、分享／匯出／刪除、離線／失敗／held-for-review。
...
| 4 | sealed-fact consumer、兩類人口統計 adapters、license ledger、四時間正規化、20 個 sealed manifest、`available_at` tests、跨 repo contract test |
...
| 6 | weekly slots、每日循環、attention／exposure／perception packet、action、三層內心、週卷 projection、commons state machine |
| 7 | model registry、選擇／fallback、strict generation、perceived-fact／truth-class／policy validator、stored-output replay |
| 8 | 完整 Web mobile＋desktop 觀測室、名冊、出生、因果、關係、公地、私人筆記、失敗狀態 |
...
- 20 個封存歷史節點各有 manifest、來源、`published／effective／available／ingested` 時間、反例、hash 與 revision-preserving 更正流程。
- 角色出生、模型選擇、20 人名冊、每週五席、每日循環、三層內心戲、證據因果、關係網、人物週卷完整可用。
...
- 玩家能遞交反證、追問未知、安排對話、寫私人筆記、追蹤公地、分享、匯出與刪除。
...
- 公開入口與觀測室通過第 12 節 performance budgets。
