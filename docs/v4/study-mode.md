...
本批正式研究 build 固定為 `study-2026-07-21.3`，同意版本固定為 `2026-07-21.v3`。production build 的 `VITE_STUDY_BUILD_ID` 未設定或不一致時，建置會直接失敗；執行階段也會在同意前再次檢查。控制台會顯示當前 build ID；混用 build ID、同意版本或 evaluator revision 的資料不產生正式裁決。
...
互動事件不會上傳，但公開 origin 必須經過 TLS 與操作員存取保護。研究代碼會出現在網址 query 中；開啟頁面時，完整網址會經邊緣存取服務送達不寫 request log 的靜態網站伺服器。邊緣服務會處理固定手機的 IP、瀏覽器資訊、開啟時間及操作員存取身分。參與者不登入，整批研究只使用操作員在固定手機上建立的存取 session。

這些連線資料只用於 TLS、存取判斷、濫用防護與故障排查，不匯入研究 JSON，也不與觀看事件合併。只有授權維運人員可查看。研究期間不得為此 zone 啟用額外 HTTP Logpush、Log Explorer 或 origin request log；存取驗證紀錄依帳戶方案最長保留 180 天，實際方案、session 到期時間與保留設定記在 repo 外的操作紀錄。研究開始前必須讓受測者看到這項說明。
...
只有至少含一個完整、有效 run 的研究代碼才計入人數；只留下損壞紀錄的代碼不會把樣本數灌到 24。必須正好 24 人、同一封存 build、同一同意版本，而且沒有損壞 run、重複觀看次序或錯日觀看，才會產生正式裁決；少於或多於 24 人都不裁決。目前 evaluator revision 為 `v4-study-3`。任一門檻未達，就回查手勢可發現性、人物辨識、事件密度與視角代價，不增加功能掩蓋結果。
