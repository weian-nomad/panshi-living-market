...
舊版 query 連結仍可讀取，僅供既有受測流程相容。公共根路徑不顯示研究說明，也不建立研究紀錄。

研究代碼只接受 `P01` 至 `P24`，代碼本身不含姓名或聯絡方式。同一位受測者兩次觀看必須使用同一代碼；不得放入姓名、電話、email 或其他可識別資料。第二次觀看必須落在第一次觀看後的下一個 `Asia/Taipei` 日曆日；同日或隔超過一天都標為不符研究規則。

研究者先完成說明與同意程序，再把裝置交給受測者。產品內的同意頁會再次說明實際記錄範圍，但不透露通過門檻、指定人物或操作答案。拒絕後不建立 run，也不寫入事件。

本批正式研究 build 固定為 `study-2026-07-23.5`，同意版本固定為 `2026-07-21.v4`。production build 的 `VITE_STUDY_BUILD_ID` 未設定或不一致時，建置會直接失敗；執行階段也會在同意前再次檢查。控制台會顯示當前 build ID；混用 build ID、同意版本或 evaluator revision 的資料不產生正式裁決。

## 記錄範圍

每個 `(appBuildId, participantCode, visitOrdinal)` 只能對應一個 `runId`，`attemptOrdinal` 永遠為 1。事件只包含：

- `run_started`：研究代碼、觀看次序、同意版本與 app build。
...
裝置內的研究事件不包含姓名、聯絡方式、IP、user agent、裝置指紋、自由文字、私人筆記或完整 pointer 軌跡。連續觀看一律使用單調時間；系統時間 `occurredAt` 只用來檢查兩次觀看是否落在相鄰的台北日曆日。

## 邊緣連線資料

互動事件不會上傳。公開 origin 以 TLS 與邊緣隧道匿名提供，不設登入或 Access gate。研究封閉性來自受控固定手機與本機匯出流程；`noindex` 與研究 query link 都不是存取控制。研究代碼會出現在網址 query 中；開啟頁面時，完整網址會經邊緣連線服務送達不寫 request log 的靜態網站伺服器。

邊緣連線服務會處理連線裝置的 IP、瀏覽器資訊與開啟時間。開啟研究頁時不進行身分驗證，參與者也不需登入。研究團隊不在應用或靜態網站伺服器保存 HTTP 請求紀錄，也不另行啟用或匯出 HTTP 請求日誌、Logpush 或 Log Explorer。服務供應商仍可能依其服務與隱私政策，為 TLS、路由、濫用防護與故障排查處理必要連線資料。這些資料不匯入研究 JSON，也不與觀看事件合併。研究代碼不是密碼，也不作為身分驗證。研究開始前必須讓受測者看到這項說明。
...
每份匯出都要在另一台受控裝置重新驗證。驗證器會檢查 schema、固定門檻、事件欄位、evaluator revision，再從原始事件重算結果；不信任 JSON 內原有的衍生數字。成功後只輸出受測總數、版本與 SHA-256，不列出逐人代碼：
...
只有至少含一個完整、有效 run 的研究代碼才計入人數；只留下損壞紀錄的代碼不會把樣本數灌到 24。必須正好 24 人、同一封存 build、同一同意版本，而且沒有損壞 run、重複觀看次序或錯日觀看，才會產生正式裁決；少於或多於 24 人都不裁決。目前 evaluator revision 為 `v4-study-4`。任一門檻未達，就回查手勢可發現性、人物辨識、事件密度與視角代價，不增加功能掩蓋結果。
