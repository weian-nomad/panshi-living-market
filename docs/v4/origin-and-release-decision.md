...
現在只部署靜態容器。主機不接收研究事件，不設應用後端、事件資料庫、analytics 或 origin request log；原始紀錄與研究控制台只存在固定手機。邊緣 TLS 與存取保護仍依 `study-mode.md` 的獨立邊界處理連線 metadata。`panshi.app` 不承載研究 build，公開根路徑不導流，研究站維持 `noindex`。
