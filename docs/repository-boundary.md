...
《盤勢・市中人》與盤勢公司市場研究是兩個獨立 repository、兩套部署與兩個 bounded context。前者消費已封存的市場事實，後者生產市場事實。任何共享都要經過可發布、可驗證、可演進的契約。
...
| 能力 | 系統紀錄來源 | 對方可做的事 |
| --- | --- | --- |
| 市場來源、授權與延遲 | 盤勢研究產品 | 遊戲只讀授權結果與 `available_at` |
...
| 記憶、關係、情緒與紙上決策 | 本 repo | 只能讀已核准的公開投影 |
| 私人筆記、匯出與刪除 | 本 repo | 不跨產品傳遞 |
| 每週席位、公地與遊戲權益 | 本 repo | 共用訂閱只能回傳最小 entitlement claim |
| 每日研究影片 | 盤勢研究產品 | 可引用經核准的公開人物片段，不讀遊戲資料庫 |
| 遊戲週卷與角色分享影片 | 本 repo | 只使用 sealed fact 與 approved narrative artifact |
...
本 repo 只接受支援的 major contract、正確 hash、已核准授權類別、`sealedAt` 非空且 `availableAt <= logicalAt` 的資料包。驗證失敗時保留上一個有效世界狀態，介面顯示延遲或暫停，不能自行補值。
